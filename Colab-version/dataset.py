...
class NIHChestXray(Dataset):
    ...
    def _construct_index(self):
        ...
        #from pathlib import Path  # ADD this import at the top if not already there!
        #paths = list(Path(self._data_path).rglob('*.png'))
        #self.names_to_path = {}
        #for path in paths:
        #    relative_path = str(path.relative_to(self._data_path))  # eg. 'images_010/00023313_005.png'
        #    self.names_to_path[relative_path] = str(path)
        self._data_path = '/content/drive/MyDrive/Colab Notebooks/images'
        paths = glob.glob(f'{self._data_path}/**/*.png', recursive=True)
        self.names_to_path = {path.split('/')[-1]: path for path in paths}
        data_entry_file = '/content/drive/MyDrive/Colab Notebooks/data/nih_chest_xrays/Data_Entry_2017.csv'
        #split_path = os.path.join(self._data_path, self._split)
        self.split_path = '/content/drive/MyDrive/Colab Notebooks/dataset_splits/train.txt'
        ...
        split_file_names = np.array([os.path.basename(file_name.strip().split(' ')[0]) for file_name in file_names])

        
        
        #df = pd.read_csv(f'{self._data_path}/{data_entry_file}')
        df = pd.read_csv(data_entry_file)
