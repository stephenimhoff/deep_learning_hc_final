...
def ten_crop_transform(crops):
        """Apply ToTensor() to all cropped images."""
...
class ChexnetTrainer(object):
    ...
    def init_dataset(self):
        ...
        train_transforms = transforms.Compose([
            transforms.RandomResizedCrop(self.args.crop),
            transforms.RandomHorizontalFlip(),
            transforms.ToTensor(),
            normalize
        ])

        # Validation & Test Transform Helpers
        ...
        # Initialize datasets
        datasetTrain = NIHChestXray(self.args, self.args.train_file, transform=train_transforms)
        datasetVal = NIHChestXray(self.args, self.args.val_file, transform=test_transforms)
        datasetTest = NIHChestXray(self.args, self.args.test_file, transform=test_transforms, classes_to_load='all')

        # Data Loaders (4,4,8 was whiat was given, adjusting to try 0)
        self.train_dl = DataLoader(dataset=datasetTrain, batch_size=self.args.batch_size, shuffle=True, num_workers=4, pin_memory=True)
        self.val_dl = DataLoader(dataset=datasetVal, batch_size=self.args.batch_size * 10, shuffle=False, num_workers=4, pin_memory=True)
        self.test_dl = DataLoader(dataset=datasetTest, batch_size=self.args.batch_size * 3, shuffle=False, num_workers=8, pin_memory=True)
    ...
    def epochVal (self):
        ...
        for i, (inputs, target) in enumerate (tqdm(self.val_dl)):
            with torch.no_grad():
                ...
                print(f"Mem: {psutil.virtual_memory().used / 1e9:.2f} GB")      
                ...
                #trying lowering ncrpos
                n_crops = 1

                varInput = torch.autograd.Variable(inputs.view(-1, c, h, w))
