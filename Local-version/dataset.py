...
class NIHChestXray(Dataset):
    ...
    def _construct_index(self):
        ...
        print(f"Looking for images in: {self._data_path}/data/nih_chest_xrays/images_*/")
        ##below line for training, final one for testing
        paths = glob.glob(f'{self._data_path}/data/nih_chest_xrays/images_*/*.png')
        # paths = glob.glob(f'{self._data_path}/images_*/*.png')
        #print('printing paths')
        #print(paths)
        #print('printing data path')
        #print(self._data_path)
        #self.names_to_path = {path.split('/')[-1]: path for path in paths}
        self.names_to_path = {os.path.basename(path): os.path.abspath(path) for path in paths}
        print(f"Found {len(self.names_to_path)} images")
        count = 0
        for key, value in self.names_to_path.items():
            print(key, value)
            count += 1
            if count == 10:
                break
        #print('printing names to path')
